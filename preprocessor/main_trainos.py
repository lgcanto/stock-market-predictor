    ...
    if not ONLY_ONE_CODE or (ONLY_ONE_CODE and row['code'] == ONLY_ONE_CODE_NAME):
        ...
        for d in range(MAX_DAYS):
            ...
            if df_company.size > 0:
                ...
                positiveSamplesInTrain = len(df_company_train)
                ...
                df_company_neutral_train = df_company_neutral.head(trainNeutralSize)
                if len(df_company_neutral_train) != 0:
                    oversamples = int(positiveSamplesInTrain/len(df_company_neutral_train))
                    df_company_neutral_os = df_company_neutral_train
                    for oversample in range(oversamples-1):
                        df_company_neutral_os = df_company_neutral_os.append(df_company_neutral_train)
                    df_company_train = df_company_train.append(df_company_neutral_os)
                ...
                exportToTSV(df_company_eval, row['code'] + '_3c_os_' + str(interval) + 'd_eval.tsv')
